...
public class TestAccumuloBinaryConverter {
  ...
  public void testInts() throws IOException {
    ...
    Assert.assertEquals(Integer.MAX_VALUE, converter.bytesToInteger(b).intValue());
  }
  
  @Test
  public void testDoubles() throws IOException {
    byte[] b = converter.toBytes(Double.MAX_VALUE);
    Assert.assertEquals(Double.MAX_VALUE, converter.bytesToDouble(b).doubleValue(), 0);
  }
  
  @Test
  public void testLongs() throws IOException {
    byte[] b = converter.toBytes(Long.MAX_VALUE);
    Assert.assertEquals(Long.MAX_VALUE, converter.bytesToLong(b).longValue());
  }
  
  @Test
  public void testFloats() throws IOException {
    byte[] b = converter.toBytes(Float.MAX_VALUE);
    Assert.assertEquals(Float.MAX_VALUE, converter.bytesToFloat(b).floatValue(), 0f);
  }
